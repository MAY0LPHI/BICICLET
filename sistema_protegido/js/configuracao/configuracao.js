...
export class ConfiguracaoManager {
    ...
    async cleanupCategoriesWithQuotes() {
        ...
        if (clientesChanged) {
            const changedClients = updatedClientes.filter((c, i) => c !== clientes[i]);
    ...
    addEventListeners() {
        ...
        const debouncedGlobalSearch = Utils.debounce((value) => {
    ...
    async restoreBackup(filename) {
        ...
        try {
            ...
            if (response.ok && result.success) {
                ...
                const errorMsg = result.error || result.errors?.slice(0, 3).join('\n') || 'Erro desconhecido';
                Modals.alert(`Erro ao restaurar backup:\n\n${errorMsg}`, 'Erro', 'alert-circle');
                this.loadBackupManagement();
            }
        } catch (error) {
    ...
    async deleteBackup(filename) {
        ...
        try {
            ...
            if (response.ok && result.success) {
                ...
                Modals.alert(result.error || 'Erro ao excluir backup', 'Aviso', 'alert-circle');
            }
        } catch (error) {
