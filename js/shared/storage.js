...
export const Storage = {
    ...
    async _performSave(clients, syncToDatabase, onProgress) {
        if (isElectron) {
            ...
            try {
                ...
                console.error('Erro ao salvar no localStorage (possível limite de espaço):', storageError);
            }
    ...
    loadClientsSync() {
        ...
        try {
            ...
            console.error('Dados de clientes corrompidos no localStorage:', e);
            return [];
        }
    ...
    async saveRegistros(registros, onProgress = null) {
        if (isElectron) {
            ...
            try {
                ...
                console.error('Erro ao salvar registros no localStorage:', storageError);
            }
    ...
    async saveRegistro(registro) {
        if (isElectron) {
            ...
            try {
                ...
                console.error('Erro ao salvar registro no localStorage:', storageError);
            }
            this.organizeRegistrosByDate(registros);
    ...
    async deleteRegistro(registroId) {
        if (isElectron) {
            const registros = await this.loadRegistros();
            const filtered = registros.filter(r => r.id !== registroId);
            await window.electron.saveRegistros(filtered);
            return { success: true };
        } else {
            const registros = this.loadRegistrosSync();
            const filtered = registros.filter(r => r.id !== registroId);
            try {
                localStorage.setItem('bicicletario_registros', JSON.stringify(filtered));
            } catch (storageError) {
                console.error('Erro ao salvar no localStorage:', storageError);
            }
            this.organizeRegistrosByDate(filtered);

            try {
                await FileStorage.deleteRegistro(registroId);
            } catch (error) {
                console.warn('Erro ao deletar registro em arquivo:', error);
            }
